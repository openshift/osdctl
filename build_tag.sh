...
function build_cmds {
  ...
  if [ -f "${bin_name}.exe" ]
  then
    mv "${bin_name}.exe" "${bin_name}-${os}-${arch}.exe"
    sha256sum "${bin_name}-${os}-${arch}.exe" > "${bin_name}-${os}-${arch}.sha256"
  else
    mv "${bin_name}" "${bin_name}-${os}-${arch}"
    sha256sum "${bin_name}-${os}-${arch}" > "${bin_name}-${os}-${arch}.sha256"
  fi
}
