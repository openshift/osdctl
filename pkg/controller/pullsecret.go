...
func FetchOwnerAccessToken(ocm *sdk.Connection, ownerUsername string, logger *logrus.Logger) ([]byte, map[string]*amv1.AccessTokenAuth, error) {
	...
	if currentAccountResp != nil && currentAccountResp.Body().Username() == ownerUsername {
		...
		// Parameter("body", nil) prevents the SDK from waiting on stdin for a POST body
...
// var (not const) so unit tests can override to avoid real sleeps
...
func UpdateHivePullSecretSSS(ctx context.Context, kubeCli client.Client, clientset kubernetes.Interface, hiveNamespace string, cdName string, pullsecret []byte, in io.Reader, out io.Writer) error {
	...
	if err := CheckExistingSyncSets(ctx, hiveNamespace, kubeCli, in, out); err != nil {
		return err
	}
	...
	if err != nil {
		...
		updateErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			existing, err = clientset.CoreV1().Secrets(hiveNamespace).Get(ctx, secretName, metav1.GetOptions{})
			if err != nil {
				return fmt.Errorf("failed to re-read secret %s/%s: %w", hiveNamespace, secretName, err)
			}
			...
			_, err = clientset.CoreV1().Secrets(hiveNamespace).Update(ctx, existing, metav1.UpdateOptions{})
			return err
		})
		if updateErr != nil {
	...
	if err := syncPullSecretViaHive(ctx, hiveNamespace, cdName, kubeCli, in, out); err != nil {
...
func syncPullSecretViaHive(ctx context.Context, hiveNamespace string, cdName string, kubeCli client.Client, in io.Reader, out io.Writer) error {
...
func UpdateHCPPullSecretViaManifestWork(ctx context.Context, ocm *sdk.Connection, kubeCli client.Client, clusterID, mgmtClusterName string, pullsecret []byte, in io.Reader, out io.Writer) error {
	...
	for round := 0; ; round++ {
		...
		if response != "y" && response != "yes" {
			...
			return fmt.Errorf("ManifestWork sync not confirmed — user aborted wait. Verify sync status manually")
		}
	}
}
