...
func TestCheckExistingSyncSets_NoExisting(t *testing.T) {
	...
	var buf bytes.Buffer
	err := CheckExistingSyncSets(context.Background(), "uhc-test-ns", kubeCli, strings.NewReader(""), &buf)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestCheckExistingSyncSets_DetectsOurSyncSet(t *testing.T) {
	...
	// Empty reader simulates user not confirming — ReadString returns EOF,
...
// --- UpdateHivePullSecretSSS mutation paths ---
// These tests verify the secret creation/update logic. The sync step (SyncSet +
// ClusterSync polling) will fail because no ClusterSync object exists, but the
// secret mutation happens before the sync call, so we can verify its state.

func init() {
	syncPollInterval = 1 * time.Millisecond
}

func TestUpdateHivePullSecretSSS_CreateSecret(t *testing.T) {
	kubeCli := fake.NewClientBuilder().WithScheme(pullSecretScheme(t)).Build()
	...
	defer cancel()
	err := UpdateHivePullSecretSSS(ctx, kubeCli, clientset,
		"uhc-test-ns", "test-cd", newPS, strings.NewReader(""), &buf)

	// Expect sync failure (no ClusterSync object), but secret should be created
	if err == nil || !strings.Contains(err.Error(), "sync to target failed") {
		t.Fatalf("expected sync failure error, got: %v", err)
	}

	secret, getErr := clientset.CoreV1().Secrets("uhc-test-ns").Get(context.Background(), "pull", metav1.GetOptions{})
	if getErr != nil {
		t.Fatalf("secret should have been created: %v", getErr)
	}
	if secret.Type != corev1.SecretTypeDockerConfigJson {
		t.Errorf("expected DockerConfigJson type, got %s", secret.Type)
	}
	if string(secret.Data[".dockerconfigjson"]) != string(newPS) {
		t.Error("secret data does not match input pull secret")
	}
}

func TestUpdateHivePullSecretSSS_UpdateExistingSecret(t *testing.T) {
	existingPS := makePullSecretJSON(map[string]map[string]string{
		"quay.io":            {"auth": "oldtok", "email": "old@b"},
		"custom.registry.io": {"auth": "custom", "email": "c@d"},
	})
	existing := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "pull", Namespace: "uhc-test-ns"},
		Type:       corev1.SecretTypeDockerConfigJson,
		Data:       map[string][]byte{".dockerconfigjson": existingPS},
	}
	...
	newPS := makePullSecretJSON(map[string]map[string]string{
		"quay.io": {"auth": "newtok", "email": "new@b"},
	})
	...
	secret, getErr := clientset.CoreV1().Secrets("uhc-test-ns").Get(context.Background(), "pull", metav1.GetOptions{})
	if getErr != nil {
		t.Fatalf("secret should exist: %v", getErr)
	}

	var parsed struct {
		Auths map[string]map[string]string `json:"auths"`
	}
	if jsonErr := json.Unmarshal(secret.Data[".dockerconfigjson"], &parsed); jsonErr != nil {
		t.Fatalf("failed to parse updated secret: %v", jsonErr)
	}

	// quay.io should be updated
	if parsed.Auths["quay.io"]["auth"] != "newtok" {
		t.Errorf("expected quay.io auth=newtok, got %s", parsed.Auths["quay.io"]["auth"])
	}
	// custom.registry.io should be preserved (merge-never-delete)
	if parsed.Auths["custom.registry.io"]["auth"] != "custom" {
		t.Errorf("expected custom.registry.io preserved, got %v", parsed.Auths["custom.registry.io"])
	}
}

func TestUpdateHivePullSecretSSS_NilDataMap(t *testing.T) {
	existing := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "pull", Namespace: "uhc-test-ns"},
		Type:       corev1.SecretTypeDockerConfigJson,
		Data:       nil, // the nil-map panic scenario
	}
	...
	if secret.Data == nil {
		t.Fatal("secret Data should not be nil after update")
	}
	if string(secret.Data[".dockerconfigjson"]) != string(newPS) {
		t.Error("secret data does not match input pull secret")
	}
	if !strings.Contains(buf.String(), "[WARN]") {
		t.Error("expected WARN about missing .dockerconfigjson key")
	}
}
