...
func (p *permissionDeniedEventsOptions) run() error {
	...
	awsAPI := NewCloudtrailClient(cfg, false)
	printer := NewPrinter(p.PrintUrl, p.PrintRaw)

	generator := awsAPI.GetEvents(p.ClusterID, startTime, time.Now().UTC())
	...
	if DefaultRegion != cfg.Region {
		...
		generator := defaultAwsAPI.GetEvents(p.ClusterID, startTime, time.Now().UTC())
