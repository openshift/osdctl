...
func (c *Cache) Read() error {
	...
	if err != nil {
		c.log.Error("failed to read cache file: %w", err)
		return err
	}
...
func (c *Cache) Save(newCache Cache) error {
	if c == nil {
		c = &Cache{
			Period: []Period{},
			Event:  []types.Event{},
	...
	allPeriods = append(allPeriods, newCache.Period...)
	...
	allEvents = append(allEvents, newCache.Event...)
	sort.Slice(allEvents, func(i, j int) bool {
		...
		// Descending order: j.Before(i) instead of i.Before(j)
...
func (p Period) DiffMultiple(c []Period) []Period {
	...
	for i, cachePeriod := range c {
		if cachePeriod.Overlap(p) {
			...
			for _, missingPeriod := range missing {
				if !missingPeriod.StartTime.Equal(missingPeriod.EndTime) && missingPeriod.periodOverlap(missingPeriods) {
...
func FilterByRegion(region string, events []types.Event) []types.Event {
	...
	for _, event := range events {
		...
		if err != nil {
			log.Errorf("failed to extract event details: %v\n", err)
			continue
		}
