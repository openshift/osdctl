...
func CheckoutAndCompareGitHash(gitexecuter iexec.IExec, gitURL, gitHash, currentGitHash string) (string, string, error) {
	...
	err = gitexecuter.Run(tempDir, "git", "clone", gitURL, "source-dir")
	if err != nil {
	...
	if gitHash == "" {
		...
		output, err := gitexecuter.Output("git", "rev-parse", "HEAD")
		if err != nil {
	...
	if currentGitHash == gitHash {
		...
		commitLog, err := gitexecuter.Output("git", "log", "--no-merges", fmt.Sprintf("%s..%s", currentGitHash, gitHash))
		if err != nil {
			return "", "", err
		}
