...
type contextData struct {
	...
	// PD Alerts
	pdServiceID      string
	PdAlerts         []pd.Incident
	HistoricalAlerts []*IncidentOccurrenceTracker
...
func (o *contextOptions) printShortOutput(data *contextData) {
	...
	highAlertCount := 0
	lowAlertCount := 0
	for _, alert := range data.PdAlerts {
	...
	if data.HistoricalAlerts != nil {
		for _, histAlert := range data.HistoricalAlerts {
...
func (o *contextOptions) generateContextData() (*contextData, []error) {
	...
	data.pdServiceID, err = GetPDSeviceID(o.baseDomain, o.usertoken, o.oauthtoken)
	if err != nil {
...
func GetCurrentPDAlertsForCluster(pdServiceID string, pdUsertoken string, pdAuthtoken string) ([]pd.Incident, error) {
	...
	for {
		...
		incidents = append(incidents, listIncidentsResponse.Incidents...)
...
func GetHistoricalPDAlertsForCluster(pdServiceID string, pdUsertoken string, pdAuthtoken string) ([]*IncidentOccurrenceTracker, error) {
...
func GetPDSeviceID(baseDomain string, usertoken string, oauthtoken string) (string, error) {
	ctx := context.TODO()
...
func printCurrentPDAlerts(incidents []pd.Incident, serviceID string) {
...
func printHistoricalPDAlertSummary(incidentCounters []*IncidentOccurrenceTracker, serviceID string, sinceDays int) error {
	...
	totalIncidents := 0
	for _, incident := range incidentCounters {
