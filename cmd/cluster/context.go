...
const (
	...
	SlackTokenConfigKey           = "slack_token"
...
func newCmdContext() *cobra.Command {
	...
	contextCmd.Flags().StringVarP(&ops.searchquery, "searchquery", "q", "", "Custom search query to grep for in slack in addition to default search pararms. By default will search for internal & external cluster id and cluster name.")
...
func (o *contextOptions) run() error {
	...
	err = o.printSlackRefs()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't print slack references: %v\n", err)
	}
	return nil
}
...
func (o *contextOptions) printSlackRefs() error {
	if !viper.IsSet(SlackTokenConfigKey) {
		return fmt.Errorf("key %s is not set in config file", SlackTokenConfigKey)
	}

	slacktoken := viper.GetString(SlackTokenConfigKey)

	slackClient := slack.New(slacktoken, slack.OptionDebug(true))

	var searchList []string
	searchList = append(searchList, o.clusterID, o.externalID, o.clusterName)
	if o.searchquery != "" {
		searchList = append(searchList, o.searchquery)
	}

	fmt.Println("============================================================")
	fmt.Println("Slack References")
	fmt.Println("============================================================")

	for _, searchQuery := range searchList {
		messages, _, err := slackClient.Search(searchQuery, slack.NewSearchParameters())
		...
		if len(messages.Matches) > 0 {
			for _, message := range messages.Matches {
				messageTime, _ := toTime(strings.Split(message.Timestamp, ".")[0])
				fmt.Printf("\t[%s] #%s [User: %s]\n", messageTime, message.Channel.Name, message.Username)
				fmt.Println("\tText:", message.Text)
				fmt.Println("\tLink:", message.Permalink)
			}
		} else {
